...
classic nginx / old-Apache model), not threads:
  ...
  - Parent: install process-wide SIGTERM/SIGINT handlers backed by a
    self-pipe, `socket_create` → `bind` → `listen`,
  ...
  - Parent: poll its self-pipe and child state, replace unexpected exits with
    backoff, and drain/reap the pool on SIGTERM or Ctrl-C. Each worker replaces
    the inherited self-pipe immediately after fork, isolating signal delivery.
